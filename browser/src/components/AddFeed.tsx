...
const AddFeed = ({user, host, code, setAddFeed}: any) => {
  ...
  const addFeed = async () => {
    ...
    const account = await new Promise<any>(res => {
    ...
    if ((account as any).subscribed === (account as any).feeds) {
      setMessage(`Account currently has a limit of ${(account as any).feeds} feeds`)
      return
    }

    setDisabledButton(true)
    setMessage("Adding feed...")
    const feed = await new Promise<any>(res => {
      user.get([host, "feeds"]).next(url, res)
    })
    if (feed && (feed as any).title) {
      user
        .get("public")
        .next("feeds")
        .next(url, userFeed => {
          if (userFeed && (userFeed as any).title) {
          ...
          const data = {
            title: (feed as any).title,
            description: (feed as any).description,
            html_url: (feed as any).html_url,
