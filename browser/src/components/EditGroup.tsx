...
const EditGroup = ({user, code, groups, currentGroup, showGroupList}: any) => {
  const [group, setGroup] = useState(() => {
    return groups.all.find(g => (g as any).key === currentGroup)
  })
  const [groupName, setGroupName] = useState(currentGroup)
  const [selected, setSelected] = useState([])
  const [message, setMessage] = useState("")
  const [disabledButton, setDisabledButton] = useState(false)
  const [feeds, updateFeed] = useReducer(reducer(null), init)

  useEffect(() => {
    if (!groups) return

    setGroup(groups.all.find(g => (g as any).key === currentGroup))
  }, [groups, currentGroup])
  ...
  useEffect(() => {
    ...
    const update = allFeeds => {
      ...
      for (const [url, f] of Object.entries(allFeeds)) {
        ...
        updateFeed({
          key: url,
          title: f ? (f as any).title : "",
          description: f ? (f as any).description : "",
          html_url: f ? (f as any).html_url : "",
          language: f ? (f as any).language : "",
          image: f ? (f as any).image : "",
        })
      }
    }
  ...
  const updateGroup = async () => {
    ...
    if ((group as any).key && (group as any).key !== groupName) {
      const removed = {
        feeds: null,
        count: 0,
        latest: 0,
      ...
      const err = await new Promise<any>(res => {
        user.get("public").next("groups").next((group as any).key).put(removed, res)
      })
      if (err) console.error(err)
    }
    user
          ...
          <List>
            {feeds &&
              feeds.all.map(
                f =>
                  (f as any).title &&
                  (group as any).feeds.includes((f as any).key) && (
                    <Feed
                      user={user}
                      code={code}
                      groups={groups}
                ...
                f =>
                  ...
                  !(group as any).feeds.includes((f as any).key) && (
                    <Feed
                      ...
                      currentGroup={currentGroup}
                      feed={f}
                      selected={selected.includes((f as any).key)}
