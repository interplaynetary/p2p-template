...
const Display = ({user, host, code, mode, setMode, feeds}: any) => {
  ...
  const loadMoreItems = useCallback(() => {
    ...
    groups.all.forEach(g => {
      g.feeds.forEach(url => {
        ...
        if (f && (f as any).items && (f as any).items[getDay.current]) {
          for (const key of Object.keys((f as any).items[getDay.current])) {
  ...
  useEffect(() => {
    ...
    setInterval(() => {
      groups.all.forEach(g => {
        g.feeds.forEach(url => {
          const f = feeds.get(url)
          if (!f || (f as any).updated < lastCheck.current) return

          if ((f as any).items && (f as any).items[today]) {
            for (const [key, item] of Object.entries((f as any).items[today])) {
              setGroupItems(current => {
                const itemKeys = new Set(current.get(g.key))
                return new Map(current.set(g.key, itemKeys.add(key)))
              })
              ...
              if ((item as any).timestamp > lastCheck.current) {
                lastCheck.current = (item as any).timestamp
                keys.push(key)
              }
              ...
              if (
                !g.latest ||
                (now < lastCheck.current && (item as any).timestamp > g.latest)
              ) {
  ...
  useEffect(() => {
    ...
    if (!itemKeys || itemKeys.size === 0) {
      ...
      setCurrentKeys(Array.from(itemKeys).sort((a, b) => Number(b) - Number(a)))
    }
