...
// I think a simpler model would be to mantain a cache of shareOfGeneralFulfillment for each typeId
// and then when we need to calculate the shareOfGeneralFulfillment for a typeId, we look it up in the cache
// if it's not in the cache, we calculate it and add it to the cache
// if it is in the cache, we return it
// and we could subscribe to this!
...
export class TreeNode {
    ...
    private typeIndexSubscription: any;
      ...
      TreeNode.fromId(typeId)
        .then(typeNode => {
          if (typeNode) {
            ...
            // If this is a contributor node (user), add reciprocal relationship
            if (typeNode.isContributor && this.isContributor) {
              console.log(`[TreeNode] Setting up reciprocal relationship with user node ${typeNode.name}`);
              
              // Add this root to the other user's type index if it doesn't exist
              if (typeNode.root.typeIndex && !typeNode.root.typeIndex.has(this.root.id)) {
                console.log(`[TreeNode] Creating reciprocal type index entry in ${typeNode.name}'s root`);
                
                // First check if the Gun reference exists (it might not if this is a newly created node)
                if (typeNode.gunRef) {
                  typeNode.gunRef.get('typeIndex').get(this.root.id).get(typeNode.id).put(true);
                }
              }
            }
    ...
    removeType(typeId: string): TreeNode {
      ...
      // Update the ID-based type index in memory
      if (root.typeIndex.has(typeId)) {
        root.typeIndex.get(typeId)?.delete(this.id);
        
        // Remove from type index in Gun - only for root nodes
        if (root.isContributor) {
          console.log(`[TreeNode] Removing type index entry: ${typeId} -> ${this.id}`);
          root.gunRef.get('typeIndex').get(typeId).get(this.id).put(null);
        }
      }
    ...
    getInstances(typeId: string): Set<TreeNode> {
      // First get instances from this root's type index
      const instanceIds = this.root.typeIndex.get(typeId) || new Set<string>();
      const instances = new Set<TreeNode>();
      
      // Special case for root nodes (users): check for reciprocal relationships
      if (this.isContributor && TreeNode.nodeCache.has(typeId) && TreeNode.nodeCache.get(typeId)?.isContributor) {
        // The target type is another user's root node
        const otherUserNode = TreeNode.nodeCache.get(typeId)!;
        
        // Check if they have us in their type index
        if (otherUserNode.typeIndex.has(this.id)) {
          console.log(`[TreeNode] Found reciprocal relationship: ${this.name} <-> ${otherUserNode.name}`);
          
          // Add all instances from the other user's type index
          otherUserNode.typeIndex.get(this.id)?.forEach(instanceId => {
            const node = TreeNode.nodeCache.get(instanceId);
            if (node) {
              instances.add(node);
            }
          });
      ...
      // Continue with regular instance resolution
    ...
    shareOfGeneralFulfillment(typeId: string): number {
      console.log(`[TreeNode] Calculating share of general fulfillment for ${this.name} with type ${typeId}`);
      
      const instances = this.getInstances(typeId);
      console.log(`[TreeNode] Found ${instances.size} instances of type ${typeId}`);
      
      if (instances.size === 0) {
        console.log(`[TreeNode] No instances found, returning 0`);
        return 0;
      }
      
      return Array.from(instances).reduce((sum, instance) => {
        console.log(`[TreeNode] Processing instance: ${instance.name} (${instance.id})`);
        ...
        if (contributorTypesCount > 0) {
          console.log(`[TreeNode] ${instance.name} has ${contributorTypesCount} contributor types:`, 
            contributorTypes.map(t => t.name).join(', '));
        } else {
          console.log(`[TreeNode] ${instance.name} has no contributor types`);
        }

        const fulfillmentWeight = instance.fulfilled * instance.weight;
        console.log(`[TreeNode] ${instance.name} fulfillment weight: ${fulfillmentWeight}`);
        ...
        console.log(`[TreeNode] ${instance.name} weight share: ${weightShare}`);
    ...
    mutualFulfillment(typeId: string): number {
      ...
      // What if we haven't loaded the type node yet? (it is the typeNode of another user)
      
      if (!typeNode) {
        console.log(`[TreeNode] Type node for ${typeId} not found`);
        return 0;
      }
    ...
    saveToGun() {
      ...
      // Save type index if this is a root node
      if (this.isContributor && this.typeIndex.size > 0) {
        console.log(`[TreeNode] Saving type index with ${this.typeIndex.size} entries`);
        
        this.typeIndex.forEach((instances, typeId) => {
          instances.forEach(instanceId => {
            console.log(`[TreeNode] Saving type index entry: ${typeId} -> ${instanceId}`);
            this.gunRef.get('typeIndex').get(typeId).get(instanceId).put(true);
          });
        });
      }
    ...
    setupSubscriptions() {
      ...
      // Add type index subscription for root nodes
      if (this.isContributor) {
        console.log(`[TreeNode] Setting up type index subscription for root node ${this.id}`);
        this.typeIndexSubscription = readFromGunPath(['nodes', this.id, 'typeIndex'], true);
        if (this.typeIndexSubscription.gunNodeRef) {
          this.typeIndexSubscription.gunNodeRef.map().map().on((data, instanceId, typeId) => {
            if (typeId === '_' || instanceId === '_') return;
            
            if (data) {
              // Add to type index in memory
              if (!this.typeIndex.has(typeId)) {
                this.typeIndex.set(typeId, new Set<string>());
              }
              this.typeIndex.get(typeId)?.add(instanceId);
              
              console.log(`[TreeNode] Type index updated: ${typeId} -> ${instanceId}`);
              this.app.pieUpdateNeeded = true;
            } else {
              // Remove from type index in memory
              if (this.typeIndex.has(typeId)) {
                this.typeIndex.get(typeId)?.delete(instanceId);
                console.log(`[TreeNode] Removed ${instanceId} from type index for ${typeId}`);
                this.app.pieUpdateNeeded = true;
              }
            }
          });
    ...
    unsubscribe() {
      ...
      // Add cleanup for type index subscription
      if (this.typeIndexSubscription && this.typeIndexSubscription.gunNodeRef) {
        this.typeIndexSubscription.gunNodeRef.off();
      }
    ...
    addToTypeIndex(typeId: string): void {
      ...
      // Persist to Gun - only for root nodes
      if (root.isContributor) {
        console.log(`[TreeNode] Persisting type index entry: ${typeId} -> ${this.id}`);
        root.gunRef.get('typeIndex').get(typeId).get(this.id).put(true);
      }
    }
