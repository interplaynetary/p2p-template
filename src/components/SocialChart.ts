...
export function createSocialChart(data: TreeNode) {
    ...
    // Get socialDistribution from root node
    console.log('Creating pie chart for data:', data);
    
    try {
        // Use the synchronous method to get the distribution
        const socialDistribution = data.socialDistribution;
        console.log('socialDistribution for pie:', socialDistribution);
        console.log('socialDistribution keys:', Array.from(socialDistribution.keys()));
        console.log('socialDistribution values:', Array.from(socialDistribution.values()));

        // Create an array of [nodeId, value] pairs for the pie chart
        const pieData: [string, number][] = Array.from(socialDistribution.entries());
        console.log('Pie chart data entries:', pieData);
        
        // If there's no data, create a placeholder pie chart
        if (pieData.length === 0) {
            console.log('No social data found, creating placeholder');
        ...
        const pie = d3.pie<[string, number]>()
            .value(d => d[1])  // Use the socialDistribution value
        ...
        // Create pie segments from socialDistribution
