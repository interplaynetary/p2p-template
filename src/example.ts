import { TreeNode } from './Free.ts';

// Export the initialization function
export async function initializeExampleData(root: TreeNode) {

    /*
        // Initialize data structure - use null parentId
        const environmentalist = new Node('environmentalist', null);
        const researcher = new Node('researcher', null, [environmentalist.id]);
        const educator = new Node('educator', null, [environmentalist.id]);

        // Our main nodes with multiple types
        const clownsWithoutBorders = new Node('clowns4all', null, [environmentalist.id]);
        const whalewatch = new Node('whalewatch', null, [researcher.id]);  // is a contributor
        const alice = new Node('alice', null, [researcher.id]);  // is a contributor
*/

        // 1. Material Needs & Dependencies
        const space = await root.addChild("Space & Environment", 25);
