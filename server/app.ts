...
const username: string = process.env.HOLSTER_USER_NAME ?? "host"
const password: string = process.env.HOLSTER_USER_PASSWORD ?? "password"
const host: string = process.env.APP_HOST ?? "http://localhost:3000"
const addFeedUrl: string | undefined = process.env.ADD_FEED_URL
const addFeedID: string | undefined = process.env.ADD_FEED_ID
const addFeedApiKey: string | undefined = process.env.ADD_FEED_API_KEY
...
const removeDays = new Set<number>()
...
app.get("/update-password", (req: Request, res: Response) => {
  res.redirect(
...
app.get("/host-public-key", (req: Request, res: Response) => {
  if (user.is) {
...
app.post("/request-invite-code", (req: Request, res: Response) => {
  if (!req.body.email) {
...
app.post("/check-codes", async (req: Request, res: Response) => {
...
app.post("/claim-invite-code", validateBody(ClaimInviteCodeRequestSchema), async (req: Request, res: Response) => {
  ...
  const account = await new Promise<Account>(res => {
...
app.post("/add-feed", validateBody(AddFeedRequestSchema), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const feed = await add.json() as unknown as FeedData
    if (feed.error) {
    ...
    user
      ...
      .put(data, (err: any) => {
        ...
        user
          .get("accounts")
          .next(code)
          .put({subscribed: (account.subscribed || 0) + 1}, (err: any) => {
            if (err) {
              console.log(err)
...
app.post("/add-subscriber", validateBody(SubscriberRequestSchema), async (req: Request, res: Response) => {
  ...
  const feed = await new Promise<Feed>(res => {
  ...
  user
    .get("feeds")
    .next(url)
    .put({subscriber_count: feed.subscriber_count + 1}, (err: any) => {
      if (err) {
        console.log(err)
...
app.post("/private/add-item", validateBody(AddItemRequestSchema), async (req: Request, res: Response) => {
  ...
  user.get("remove").next(removeKey, async (remove: Record<string, RemoveItem>) => {
    if (!remove) return
...
function mapEnclosure(e: any): any {
  if (!e) return null
...
function day(key: number): number {
  const t = new Date(+key)
  return Date.UTC(t.getUTCFullYear(), t.getUTCMonth(), t.getUTCDate())
}

async function checkCodes(newCodes: string[]): Promise<boolean> {
...
async function checkHosts(newCodes: string[]): Promise<boolean> {
...
async function createInviteCodes(count: number, owner: string, account: any): Promise<boolean> {
  let i = 0
  let newCodes = []
  while (i++ < count) {
...
function requestInvite(email: string): void {
...
function sendInviteCode(code: string, email: string): void {
...
function validateEmail(name: string, email: string, code: string, validate: string): void {
...
function resetPassword(name: string, remaining: number, email: string, code: string, reset: string): void {
  const message = `Hello ${name}
...
function mail(email: string, subject: string, message: string, bcc?: string): void {
